...
1. Apply to a shape layer, set **Background Layer** to the layer to
   refract. That's it.
2. Full composite (no precomp): put the effect on an **adjustment layer**,
   check **Sample underlying layers**, and set **Glass Shape** to a shape
   layer (its video switch may be off) — this might result in a different look.
   Important: the Shape layer picked as "Glass Shape" must NOT have the
   Adjustment Layer switch enabled — AE hands such layers to effects as a
   white solid, which flattens the glass.
